...
function renderDashboard(target) {
  ...
  // Cards Superiores Informativos
  const grid = document.createElement('div')
  grid.className = 'summary-grid'
  grid.innerHTML = `
        <div class="summary-card" style="background-color: ${balance >= 0 ? CORES.greenLight : CORES.redLight}; color: ${balance >= 0 ? CORES.green : CORES.red}">
            <div class="summary-label">Saldo livre</div>
            <div class="summary-value">${fmt(balance)}</div>
        </div>
        <div class="summary-card" style="background-color: ${CORES.greenLight}; color: ${CORES.green}">
        ...
        <div class="summary-card" style="background-color: ${CORES.accentLight}; color: ${CORES.accent}">
...
function createMovementRowElement(m, showDelete = true) {
  ...
  if (showDelete) {
    row.querySelector('.btn-delete-row').addEventListener('click', (e) => {
      ...
      state.data.movements = state.data.movements.filter((mov) => mov.id !== id)
      saveState()
